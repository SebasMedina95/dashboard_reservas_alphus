...
class ModeloTestimonios{
	...
	static public function mdlMostrarTestimoniosInnerJoin($tabla1, $tabla2, $tabla3, $item, $valor){
			...
			$stmt = Conexion::conectar()->prepare("SELECT $tabla1.*, $tabla2.*, $tabla3.* FROM $tabla1 INNER JOIN $tabla2 ON $tabla1.id_reserva_t = $tabla2.id_reserva INNER JOIN $tabla3 ON $tabla1.id_usuario_t = $tabla3.id_u WHERE $item = :$item");
